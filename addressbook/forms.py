...
class FindContactsForm(forms.Form):
    '''for template addressbook/find_contacts'''
    ...
    pattern = forms.CharField(label='паттерн', required=False,
            help_text='набор символов, которым будет искаться соотвествие \
                (в имени, тпелефонах, адресах, email, заметках)')
    tags = forms.MultipleChoiceField(
        choices=tags_test, widget=forms.CheckboxSelectMultiple,
        label='выберите тэги', required=False,
        help_text='можно выбрать один или несколько тэгов, \
